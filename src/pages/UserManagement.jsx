...
// Thêm prop onRefresh và currentUserId vào component
const UserManagementPage = ({ onRefresh, currentUserId }) => { 
  ...
  // SỬ DỤNG useCallback ĐỂ ĐẢM BẢO KHÔNG BỊ LỖI STALE CLOSURE
  const handleUserUpdated = useCallback((updatedUserId) => {
    setUpdateModalOpen(false);
    setToast({ message: 'Cập nhật người dùng thành công!', type: 'success' });
    fetchAllData(); // Cập nhật danh sách người dùng trên trang Admin
    
    // Kích hoạt Refresh nếu tài khoản được cập nhật là tài khoản đang đăng nhập
    if (onRefresh && updatedUserId === currentUserId) {
      onRefresh(); // Kích hoạt fetchUserInfo trong App.jsx
    }
  }, [onRefresh, currentUserId]); // Dependency array đảm bảo sử dụng giá trị mới nhất của props
    ...
    <>
      ...
      {isUpdateModalOpen && (
        <UpdateUserModal
          ...
          roles={allRoles}
          onClose={() => setUpdateModalOpen(false)}
          // TRUYỀN ID CỦA NGƯỜI ĐƯỢC CẬP NHẬT LÊN
