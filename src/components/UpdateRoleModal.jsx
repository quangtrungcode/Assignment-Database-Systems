...
const UpdateRoleModal = ({ role, allPermissions, onClose, onRoleUpdated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Xử lý các trường dữ liệu text (Chuyển rỗng thành null nếu cần)
      ...
      // 2. Lấy danh sách Permissions hiện tại đang được chọn
      // QUAN TRỌNG: Luôn lấy list này để gửi đi, không dùng điều kiện if check thay đổi nữa
      const permissionsToSubmit = Array.from(selectedPermissionsSet).map(uniqueKey => {
        const found = processedPermissions.find(p => p.uniqueKey === uniqueKey);
        // Ưu tiên lấy tên gốc (originalName) vì backend lưu theo tên quyền
        return found ? found.originalName : uniqueKey;
      });

      // 3. Tạo Payload (Body request)
      const payload = { 
        // Backend không dùng id trong body để update (vì dùng roleName trên URL), 
        // nhưng ta vẫn nên gửi hoặc để null an toàn.
        id: role.id || null, 
        
        roleName: name, // Tên vai trò (để đảm bảo tính nhất quán)
        description: finalDescription, 
        level: finalLevel,
        permissions: permissionsToSubmit // 🔥 FIX LỖI: Luôn gửi mảng quyền (không được để null/undefined)
      };

      console.log('Update Role Payload:', payload);

      // 4. Gọi API
      // Tham số 1: name (chính là roleId trên URL như bạn nói)
      // Tham số 2: payload (dữ liệu body)
